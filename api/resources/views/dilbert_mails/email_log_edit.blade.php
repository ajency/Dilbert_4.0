                	...
                	<p>{{date('F jS, Y', strtotime($user_data['modified_on']))}}</p>
						@foreach($user_data['values'] as $values)
		                    ...
		                    @elseif($values['column_modified']=="start_time" || $values['column_modified']=="end_time")
	                    		...
	                    		@if($values['new_value']=="" && $values['old_value']=="" || date(strtotime($values['new_value']))=="00:00" && date(strtotime($values['old_value']))=="00:00")
		                    		00:00 to 00:00
			                    @elseif($values['new_value']=="" || date(strtotime($values['new_value']))=="00:00")
			                    	{{date('h:i:s a m/d/Y', strtotime($values['old_value']))}}
			                    	to
			                    	00:00
			                     @elseif($values['old_value']=="" || date(strtotime($values['old_value']))=="00:00")
			                    	00:00
			                    	to
